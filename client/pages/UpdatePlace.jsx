...
const UpdatePlace = () => { 
    ...
    useEffect( () => { 
        ...
        const renderData = async () => { 
            const cres = await fetchData("http://localhost:3000/api/cities"); 
            setApiCities(cres.cities); 
            ...
            await setType(res.type); 
            await setHistory(res.history); 
            await setContact(res.contact); 
            await setCity(res.city); 
    ...
    return( 
        <section className="form">
            <form method="POST" action =  { `http://localhost:3000/api/${location.pathname}` }  onSubmit={() => { handleSubmit(submitForm)}}>
